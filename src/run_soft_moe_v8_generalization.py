# -*- coding: utf-8 -*-
"""
run_soft_moe_v5_tabnet.py
========================================================================
Revision history:
  v3: R1/R2/R3/R4/R6 reviewer fixes (MLP baseline, K-sensitivity, CV caveat).
  v4: Round-2 reviewer fixes (ج1-ج5, Holm, 95% CI, per-fold K-sens, gate stability).
  v5 (this file):
    • GlobalMLP baseline → TabNet (Arik & Pfister, 2021). TabNetClassifier
      is a stronger, attention-based tabular baseline providing fairer
      comparison against the soft-MoE multi-expert framework.
    • TabNet hyperparameters: n_d=n_a=64, n_steps=5, γ=1.5, λ_sparse=1e-3,
      mask_type='sparsemax', balanced class weighting (weights=1).
    • Four new publication-ready comparison figures:
        Fig H — AUC-ROC per-repeat comparison (Table 9 equivalent)
    ...
    • All stat comparisons updated: global_mlp → tabnet.
  v6:
    • Added an LSTM baseline, addressing the Limitations statement that
      "the architecture has not been compared against sequence-native
      transformers/temporal models". Since the released KDD Cup 2015
      dataset provides only aggregated per-enrollment tabular features
      (no raw per-event timestamped sequence), the 490-dim feature
      vector is chunked into LSTM_SEQ_LEN contiguous, equal-width
      pseudo-timesteps (in the original column order) and fed to a
      single-layer LSTM. This is a "sequentialized-tabular" LSTM, not a
      true recurrent model over raw clickstream events — the chunking
      design choice is stated explicitly here and should be reported
      identically in the paper for transparency.
    • Added a fold-matched, GroupKFold-based learner-/course-held-out
      generalization protocol (R1#1) and per-fold gate/cluster Adjusted
      Rand Index logging (R2#4) — see the bottom of this file.
  v7 (this file) — professional refactor of the R1#1 generalization suite:
    • NEW: temporal-holdout protocol (run_temporal_holdout_protocol), the
      third leg of R1#1 that was previously missing entirely. Sorts all
      enrollments chronologically by an auto-detected (or explicitly
      passed) timestamp column and trains on the earliest --train-frac
      (default 80%) while testing on the most recent remainder — a
      single, non-shuffled split, because temporal order is not
      exchangeable. Mirrors the SHAP-cluster-discovery + SoftMoE +
      isotonic-calibration pipeline used everywhere else in this file.
    • The three R1#1 protocols (learner / course / temporal) are now
      unified behind one dispatcher, run_holdout_suite(), driven by a
      single HOLDOUT_PROTOCOL_REGISTRY so adding a fourth protocol in
      the future is a one-entry change, not a copy-pasted function.
    • CLI behavior change (documented, not silent): running
        python <this_script>.py --holdout both
      now runs ONLY the requested holdout protocol(s) — main() is
      SKIPPED by default — so the exact command used to request
      learner+course held-out results returns
      holdout_learner_held_out_results.csv and
      holdout_course_held_out_results.csv directly, without first
      waiting through the ~18-22h main 25-run study. Pass
      --also-run-main to restore the old "run everything" behavior.
    • New CLI flags: --holdout temporal, --holdout all (learner+course+
      temporal in one call), --k-folds, --train-frac, --time-col,
      --data-path, --cluster-path — all with sensible defaults so the
      exact previously-documented commands keep working unchanged.
    • Minor robustness/perf cleanup in the holdout protocols: group/row
      membership tests now use np.isin() instead of Python-level set
      comprehensions; consistent docstrings; explicit, actionable error
      messages when a requested column is not found in the data.
  v8 (this file) — default entry point now runs the R1#1 suite directly:
    • Changed --holdout's default from "none" to "all". Simply running
      this script with NO arguments (python <this_script>.py, or running
      the cell directly in Jupyter/Colab, as in the earlier v5/v6/v7
      workflow) now executes all three R1#1 protocols (learner-held-out +
      course-held-out + temporal-holdout) directly and skips the main
      25-run study — no CLI flags needed. This matches how the script is
      actually being invoked in practice (a notebook cell / bare `python
      script.py`, where passing flags is inconvenient) and avoids
      accidentally re-running the ~18-22h main study when only the R1#1
      robustness check was wanted.
    • To run the main study instead, either call main() directly, or run
      with `--holdout none --also-run-main` (equivalently, import this
      module and call main() from your own driver code).
========================================================================
فكرة بحثية: SHAP-Guided Soft Mixture-of-Experts (MoE)
------------------------------------------------------------------------
المشكلة التي حلّها هذا التصميم:
  - النموذج العام الواحد: يحافظ على AUC لكنه لا يستغل أنماط العناقيد.
  - التوجيه الصلب (Hard Routing) للخطوة السابقة: حسّن F1/Recall لكنه
    أضعف AUC بشكل دالّ إحصائيًا (p=0.0014)، لأنه جزّأ بيانات التدريب
    على 3 نماذج صغيرة منفصلة، فقلّ حجم العيّنة لكل خبير.

الحل المُقترح:
  - 3 "خبراء" (شبكات عصبية صغيرة)، كل خبير يرى **كل** البيانات في كل
    Batch (لا تجزيء صارم) -> لا تُفقَد القوة الإحصائية.
  - "بوّابة" (Gating Network) تُخرِج وزن مزج ناعم (Softmax) بين الخبراء
    لكل طالب، تتعلّم بشكل مشترك (End-to-End) مع الخبراء عبر BCE.
  - توجيه لطيف (لا صارم) للبوّابة نحو عناقيد SHAP المُكتشَفة مسبقًا، عبر
    حدّ تنظيم (KL Divergence) بين توزيع البوّابة وتوزيع مُصنِّف العنقود
    البديل (Surrogate)، بمعامل صغير (λ) يُتيح للبوّابة حرية التعديل
    إن كان مفيدًا للمهمة الأساسية.

  التنبؤ النهائي = Σ (وزن البوّابة لكل خبير) × (احتمال ذلك الخبير)

كل شيء (تدريب Surrogate + Soft-MoE) يحدث **داخل** كل fold من 5-Fold CV
لتفادي أي تسرّب، ولإجراء مقارنة مُزاوَجة (paired) صحيحة مع النموذج العام
بنفس تقسيمات الفولدات بالضبط.

المخرجات:
  soft_moe_cv_raw.csv            (نتائج كل fold: عام / Soft-MoE)
  soft_moe_cv_summary.csv        (متوسط/انحراف معياري)
  soft_moe_stat_comparison.json  (اختبار إحصائي مُزاوَج لكل المقاييس)
"""
...
try:
    ...
    print("[v5] pytorch_tabnet not found — installing via pip ...")
    subprocess.check_call(
        [sys.executable, "-m", "pip", "install", "pytorch-tabnet",
         "--quiet", "--break-system-packages"],
        stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
    )
    print("[v5] pytorch_tabnet installed successfully.")
# ─────────────────────────────────────────────────────────────────────────────
...
# v7 — R1#1 generalization-protocol defaults. Column names for the
# learner-held-out / course-held-out protocols are passed explicitly at
# the CLI (see HOLDOUT_PROTOCOL_REGISTRY below); the temporal-holdout
# protocol instead AUTO-DETECTS a timestamp column from this candidate
# list (case-insensitive), since no single obvious column name is
# guaranteed to exist in every processed export of this dataset. Add
# your own column name here, or pass --time-col explicitly, if none of
# these match.
TIME_COLUMN_CANDIDATES = [
    "enrollment_time", "enroll_time", "start_time", "course_start",
    "first_event_time", "enrollment_date", "enroll_date", "date", "time",
    "created_at", "timestamp",
]
HOLDOUT_K_FOLDS_DEFAULT = 5        # GroupKFold splits for learner/course protocols
HOLDOUT_TRAIN_FRAC_DEFAULT = 0.8   # earliest-fraction trained on in temporal holdout
...
def full_calibration_report(y_true, y_prob, n_bins=10):
    """يُجمِّع كل مقاييس المعايرة الأربعة في قاموس واحد لتسهيل التسجيل في all_rows."""
...
def main():
    ...
    extra_cal_cols = [c for c in ["brier", "nll", "calib_slope", "calib_intercept"] if c in res_df.columns]
...
# ============================================================================
#   R1#1 — GENERALIZATION-ROBUSTNESS PROTOCOL SUITE
#   (learner-held-out / course-held-out / temporal-holdout)
# ============================================================================
...
# that a given learner (via multiple enrollments) or course is absent from
# both the training and test partition of a fold, nor does it test whether
# performance holds when training only on past cohorts and testing on
# future ones. The three protocols below re-evaluate the two headline
# models — the fold-matched global LightGBM baseline and SoftMoE+Isotonic
# — under splits that DO enforce these guarantees:
#
#   • learner_held_out  — GroupKFold on the learner id column ("username"):
#                          no learner appears in both train and test of a fold.
#   • course_held_out   — GroupKFold on the course id column
#                          ("course_id_encoded"): no course appears in both
#                          train and test of a fold.
#   • temporal_holdout  — a SINGLE chronological split: train on the
#                          earliest --train-frac (default 80%) of
#                          enrollments, test on the most recent remainder.
#                          Not a k-fold, because temporal order is not
#                          exchangeable — this mimics deploying the model
#                          on a future cohort/course offering.
#
...
#     compared here — not the full ablation suite (TabNet, LSTM, hard
#     routing, lambda=0) from Table 8. This is a targeted robustness check
#     on the paper's central claim, not a full replication of every table.
#   • learner_held_out / course_held_out use K=5 folds, no repeats (vs.
#     5x5=25 in the main study), to keep runtime tractable: roughly 1/5 of
#     the main study's wall-clock time per protocol. temporal_holdout is a
#     single split, so it is faster still (roughly 1/25).
#   • The SHAP-cluster discovery and surrogate-gate-prior pipeline
#     (Section IV.A) is preserved unchanged inside each fold/split, exactly
#     as in main(), so these protocols test generalization of the *same*
#     architecture and training procedure, not a simplified variant.
#   • A hard assertion verifies zero group overlap between train and test
#     in every learner/course fold before any model is fit.
#
# CLI usage (this file can be run directly):
#   python run_soft_moe_v7_generalization.py --holdout learner
#   python run_soft_moe_v7_generalization.py --holdout course
#   python run_soft_moe_v7_generalization.py --holdout temporal
#   python run_soft_moe_v7_generalization.py --holdout both     # learner + course
#   python run_soft_moe_v7_generalization.py --holdout all      # + temporal
#
# IMPORTANT — behavior change vs. earlier releases of this file: by
# default, passing --holdout runs ONLY the requested protocol(s); the main
# 25-run study is SKIPPED so you get the requested CSV(s) back quickly
# instead of waiting ~18-22h. Pass --also-run-main to run both.
# ============================================================================


def _autodetect_time_column(df, candidates=TIME_COLUMN_CANDIDATES):
    """Case-insensitive lookup of the first candidate column name present
    in `df`. Returns None if none match — callers surface an actionable
    error rather than silently guessing, since fabricating a temporal
    ordering would invalidate the whole point of this protocol."""
...
def _load_features_and_clusters(data_path, cluster_path):
    """Shared data-loading/feature-preparation step used by every R1#1
    protocol below. Mirrors main()'s loading logic exactly (same dedup
    rule, same numeric-only feature selection, same fillna/clip policy) so
    that these protocols evaluate the identical feature representation
    used for the paper's primary claims — only the train/test split
    strategy differs.

    Returns
    -------
    df : pd.DataFrame            (full raw frame, including non-feature columns)
    X_raw : np.ndarray           (n_samples, n_features), float32
    y : np.ndarray                (n_samples,), int
    feature_cols : list[str]
    labeled_rows_all : np.ndarray  (row indices with a Phase-1 SHAP cluster label)
    labeled_clusters_all : np.ndarray (their cluster labels c(x))
    """
    df = pd.read_csv(data_path)
    ...
    dup_cols = df[feature_cols].columns[dup_mask].tolist()
    feature_cols = [c for c in feature_cols if c not in dup_cols]
    print(f"  [R1#1] {len(feature_cols)} numeric feature columns "
          f"(after removing {len(dup_cols)} duplicates), matching main().")
    ...
    return df, X_raw, y, feature_cols, labeled_rows_all, labeled_clusters_all


def _fit_surrogate_gate_prior(X_raw, tr_idx, tr_sub_idx, va_idx,
                               labeled_rows_all, labeled_clusters_all,
                               in_dim, seed=SEED, min_labeled=30):
    """Shared fold-internal surrogate-gate-prior construction, used
    identically by every R1#1 protocol below and by main(): fits the
    multiclass LightGBM surrogate q(x) ~= P(c|x) on the at-risk students
    that fall inside THIS split's training partition (tr_idx) only, then
    infers soft cluster-probability targets for the train-sub/validation
    rows used to train SoftMoE. Falls back to a uniform prior (equivalent
    to lambda=0 for this split) if too few labeled samples are available
    inside the training partition — this can happen in small or unusually
    imbalanced group-held-out folds.

    Returns (gate_target_tr, gate_target_va, used_uniform_fallback: bool).
    """
    mask_in_train = np.isin(labeled_rows_all, tr_idx)
    rows_for_surrogate = labeled_rows_all[mask_in_train]
    clusters_for_surrogate = labeled_clusters_all[mask_in_train]

    if len(rows_for_surrogate) < min_labeled or len(np.unique(clusters_for_surrogate)) < 2:
        print(f"    [R1#1 WARN] Only {len(rows_for_surrogate)} labeled at-risk samples "
              f"in this split's training partition — falling back to a uniform gate "
              f"prior (equivalent to lambda=0 for this split).")
        ...
        return gate_target_tr, gate_target_va, True
    ...
    del surrogate
    gc.collect()
    return gate_target_tr, gate_target_va, False


def _fit_and_score_global_and_softmoe(X_raw, y, tr_sub_idx, va_idx, te_idx,
                                       gate_target_tr, gate_target_va,
                                       in_dim, fold_seed=0):
    """Shared model-fitting/scoring step for a single split (fold or the
    single temporal split): trains the fold-matched LightGBM baseline and
    SoftMoE+Isotonic with the identical architecture/training procedure
    used in main(), and returns their evaluation-metric dicts.

    Returns (res_global, ece_global, res_moe_iso, ece_moe_iso).
    """
    y_tr, y_va, y_te = y[tr_sub_idx], y[va_idx], y[te_idx]

    # ---- LightGBM global baseline (identical config to main()) ----
    model_global = fit_lgb(X_raw[tr_sub_idx], y_tr, X_raw[va_idx], y_va, GLOBAL_PARAMS)
    p_global_va = model_global.predict_proba(X_raw[va_idx])[:, 1]
    thr_global = best_threshold(y_va, p_global_va)
    p_global_te = model_global.predict_proba(X_raw[te_idx])[:, 1]
    ...
    # ---- SoftMoE + Isotonic (identical architecture/training to main()) ----
    scaler = StandardScaler().fit(X_raw[tr_sub_idx])
    X_tr_s = scaler.transform(X_raw[tr_sub_idx]).astype(np.float32)
    X_va_s = scaler.transform(X_raw[va_idx]).astype(np.float32)
    X_te_s = scaler.transform(X_raw[te_idx]).astype(np.float32)
    ...
    moe_model = train_soft_moe(X_tr_s, y_tr, gate_target_tr, X_va_s, y_va, gate_target_va,
                                in_dim, pos_w, fold_seed=fold_seed)
    with torch.no_grad():
        p_moe_va, _, _ = moe_model(torch.tensor(X_va_s, dtype=torch.float32, device=DEVICE))
    ...
    return res_global, ece_global, res_moe_iso, ece_moe_iso


def run_group_holdout_protocol(group_col, protocol_name, k_folds=HOLDOUT_K_FOLDS_DEFAULT,
                                data_path=DATA_PATH, cluster_path=CLUSTER_ASSIGN_PATH,
                                seed=SEED):
    """
    ...
    data_path, cluster_path : str
        Overridable paths, defaulting to the same files main() uses.
    ...
    df, X_raw, y, feature_cols, labeled_rows_all, labeled_clusters_all = \
        _load_features_and_clusters(data_path, cluster_path)

    if group_col not in df.columns:
        raise ValueError(
            f"[R1#1 HOLDOUT PROTOCOL] Column '{group_col}' not found in "
            f"{data_path}. Available columns (first 25): "
            f"{list(df.columns[:25])} ... "
            f"Please confirm the exact raw-data column name for this "
            f"identifier (e.g. it may be 'course_id' rather than "
            f"'course_id_encoded' in the raw, pre-encoding CSV) and pass "
            f"it explicitly via --group-col / group_col."
        )
    ...
    if n_unique_groups < k_folds:
        raise ValueError(
            ...
            f"--k-folds or check that '{group_col}' is the intended "
            f"grouping column."
        )
    del df
    gc.collect()
    ...
    for fold_id, (tr_idx, te_idx) in enumerate(gkf.split(X_raw, y, groups)):
        ...
        gate_target_tr, gate_target_va, used_fallback = _fit_surrogate_gate_prior(
            X_raw, tr_idx, tr_sub_idx, va_idx, labeled_rows_all, labeled_clusters_all, in_dim, seed=seed)

        res_global, ece_global, res_moe_iso, ece_moe_iso = _fit_and_score_global_and_softmoe(
            X_raw, y, tr_sub_idx, va_idx, te_idx, gate_target_tr, gate_target_va,
            in_dim, fold_seed=fold_id)
        ...
        rows.append({"protocol": protocol_name, "fold": fold_id, "model": "global",
                      ...
                      "uniform_gate_fallback": used_fallback,
...
def run_temporal_holdout_protocol(time_col=None, protocol_name="temporal_holdout",
                                   train_frac=HOLDOUT_TRAIN_FRAC_DEFAULT,
                                   data_path=DATA_PATH, cluster_path=CLUSTER_ASSIGN_PATH,
                                   seed=SEED):
    """
    Re-evaluate the global LightGBM baseline and SoftMoE+Isotonic under a
    SINGLE chronological split: train on the earliest `train_frac`
    (default 80%) of enrollments (sorted by `time_col`), test on the most
    recent remainder. Unlike the learner/course protocols this is NOT a
    k-fold split — temporal order is not exchangeable, so shuffling it
    into folds would defeat the purpose of the check. This mimics the
    real deployment scenario of training on past cohorts and predicting
    dropout for a future cohort/course offering.

    Parameters
    ----------
    time_col : str or None
        Raw-data column to sort by. If None, auto-detected from
        TIME_COLUMN_CANDIDATES (case-insensitive). Accepts either a
        parseable datetime string column or an already-numeric/ordinal
        timestamp column.
    protocol_name : str
        Label used in printed output and the output CSV filename.
    train_frac : float
        Fraction of chronologically-earliest enrollments used for
        training (the rest — the most recent enrollments — is the test set).
    data_path, cluster_path : str
        Overridable paths, defaulting to the same files main() uses.
    ...
    -------
    pandas.DataFrame
        One row per model ("global", "soft_moe_isotonic"), also saved to
        f"holdout_{protocol_name}_results.csv".

    Raises
    ------
    ValueError
        If no timestamp column can be found or resolved, or if the
        resolved column contains unparseable values — this protocol
        refuses to silently fabricate a temporal ordering.
    """
    t_start = time.time()
    print(f"\n{'='*70}\n[R1#1 HOLDOUT PROTOCOL] {protocol_name}  "
          f"(chronological {train_frac*100:.0f}/{100-train_frac*100:.0f} split, "
          f"single pass)\n{'='*70}")
    ...
    resolved_time_col = time_col or _autodetect_time_column(df)
    if resolved_time_col is None:
        script_name = os.path.basename(sys.argv[0]) or "run_soft_moe_v7_generalization.py"
        raise ValueError(
            f"[R1#1 HOLDOUT PROTOCOL] No timestamp column found automatically "
            f"among candidates {TIME_COLUMN_CANDIDATES}. If your CSV has an "
            f"enrollment/event timestamp under a different name, pass it "
            f"explicitly, e.g.:\n"
            f"  python {script_name} --holdout temporal --time-col <column_name>\n"
            f"Available columns (first 30): {list(df.columns[:30])}"
        )
    if resolved_time_col not in df.columns:
        raise ValueError(
            f"[R1#1 HOLDOUT PROTOCOL] Explicit --time-col '{resolved_time_col}' "
            f"not found in {data_path}. Available columns (first 30): "
            f"{list(df.columns[:30])}"
        )
    print(f"  [R1#1] Using time column: '{resolved_time_col}'")

    try:
        time_values = pd.to_datetime(df[resolved_time_col]).astype("int64").to_numpy()
    except (ValueError, TypeError):
        time_values = pd.to_numeric(df[resolved_time_col], errors="coerce").to_numpy()

    n_bad = int(np.isnan(time_values).sum()) if np.issubdtype(time_values.dtype, np.floating) else 0
    if n_bad > 0:
        raise ValueError(
            f"[R1#1 HOLDOUT PROTOCOL] Column '{resolved_time_col}' has {n_bad:,} "
            f"unparseable/missing values out of {len(time_values):,} rows — "
            f"refusing to guess a temporal ordering. Clean this column or "
            f"pick a different --time-col."
        )
    del df
    gc.collect()
    ...
    tr_idx_full = order[:cut]
    te_idx = order[cut:]
    print(f"  [R1#1] Chronological split: train={len(tr_idx_full):,} "
          f"(earliest {train_frac*100:.0f}%), test={len(te_idx):,} "
          f"(most recent {100*(1-train_frac):.0f}%)")

    tr_sub_idx, va_idx = train_test_split(
        tr_idx_full, test_size=0.15, stratify=y[tr_idx_full], random_state=seed)

    gate_target_tr, gate_target_va, used_fallback = _fit_surrogate_gate_prior(
        X_raw, tr_idx_full, tr_sub_idx, va_idx, labeled_rows_all, labeled_clusters_all,
        in_dim, seed=seed)
    ...
    rows = [
        {"protocol": protocol_name, "fold": 0, "model": "global",
         "n_train": len(tr_idx_full), "n_test": len(te_idx),
         "time_col": resolved_time_col, "train_frac": train_frac,
         "uniform_gate_fallback": used_fallback,
         "ece_after_isotonic": ece_global, **res_global},
        {"protocol": protocol_name, "fold": 0, "model": "soft_moe_isotonic",
         "n_train": len(tr_idx_full), "n_test": len(te_idx),
         "time_col": resolved_time_col, "train_frac": train_frac,
         "uniform_gate_fallback": used_fallback,
         "ece_after_isotonic": ece_moe_iso, **res_moe_iso},
    ...
    print(f"\n  [R1#1 HOLDOUT PROTOCOL] {protocol_name} \u2014 single-split summary "
          f"({time.time()-t_start:.0f}s total):")
    print(result_df[["model", "auc", "f1", "mcc", "accuracy", "precision", "recall"]]
          .set_index("model").to_string())
    print(f"  File saved: {out_path}")
...
# ─────────────────────────────────────────────────────────────────────────────
# v7 — Unified R1#1 dispatcher. Every protocol is registered here once, so
# adding a future protocol (e.g. a second temporal split, or an
# institution-held-out check) is a one-entry change rather than a new
# copy-pasted CLI branch.
# ─────────────────────────────────────────────────────────────────────────────
HOLDOUT_PROTOCOL_REGISTRY = {
    "learner":  dict(kind="group", group_col="username", protocol_name="learner_held_out"),
    "course":   dict(kind="group", group_col="course_id_encoded", protocol_name="course_held_out"),
    "temporal": dict(kind="temporal", protocol_name="temporal_holdout"),
}
# CLI-facing aliases that expand to a set of registry keys.
HOLDOUT_CLI_ALIASES = {
    "learner": {"learner"},
    "course": {"course"},
    "temporal": {"temporal"},
    "both": {"learner", "course"},                # legacy alias, preserved
    "all": {"learner", "course", "temporal"},
    "none": set(),
}


def run_holdout_suite(which, k_folds=HOLDOUT_K_FOLDS_DEFAULT, time_col=None,
                       train_frac=HOLDOUT_TRAIN_FRAC_DEFAULT,
                       data_path=DATA_PATH, cluster_path=CLUSTER_ASSIGN_PATH):
    """
    Run one or more R1#1 generalization protocols (learner / course /
    temporal) and print a combined comparison against the main-study
    (enrollment-level) reference numbers reported in Table 8.

    Parameters
    ----------
    which : iterable[str]
        Subset of {"learner", "course", "temporal"} (see
        HOLDOUT_PROTOCOL_REGISTRY). Order is not significant; each
        protocol is run independently and its results are unaffected by
        the others.

    Returns
    -------
    dict[str, pandas.DataFrame]
        Keyed by protocol_name (e.g. "learner_held_out"), each also saved
        individually to f"holdout_{protocol_name}_results.csv". All
        results are additionally concatenated and saved to
        "holdout_all_protocols_combined.csv".
    """
    which = [k for k in HOLDOUT_PROTOCOL_REGISTRY if k in set(which)]  # stable, registry order
    if not which:
        print("[R1#1] run_holdout_suite() called with no protocols selected — nothing to do.")
        return {}

    results = {}
    for key in which:
        spec = HOLDOUT_PROTOCOL_REGISTRY[key]
        try:
            if spec["kind"] == "group":
                df_res = run_group_holdout_protocol(
                    group_col=spec["group_col"], protocol_name=spec["protocol_name"],
                    k_folds=k_folds, data_path=data_path, cluster_path=cluster_path)
            else:  # "temporal"
                df_res = run_temporal_holdout_protocol(
                    time_col=time_col, protocol_name=spec["protocol_name"],
                    train_frac=train_frac, data_path=data_path, cluster_path=cluster_path)
            results[spec["protocol_name"]] = df_res
        except Exception as e:
            # Isolate failures per protocol (e.g. temporal-holdout raising
            # because no timestamp column could be found) so that other,
            # independent protocols in this run still complete and their
            # already-computed CSVs are not lost. This matters especially
            # now that --holdout defaults to "all".
            print(f"\n[R1#1 ERROR] Protocol '{spec['protocol_name']}' failed and was "
                  f"skipped: {e!r}")
            print(f"[R1#1 ERROR] The other requested protocol(s) will still run.")

    print(f"\n{'='*70}\n[R1#1] COMBINED SUMMARY: enrollment-level vs. generalization-robustness "
          f"protocols\n{'='*70}")
    ...
    for name, df_res in results.items():
        ...
        n_units = df_res["fold"].nunique()
        unit_label = "fold" if n_units > 1 else "split"
        print(f"\n  {name} (this run, {n_units} {unit_label}{'s' if n_units != 1 else ''}):")
    ...
    if not results:
        print("  [R1#1 WARN] All requested protocols failed — see [R1#1 ERROR] messages above.")
    return results


def _build_arg_parser():
    """Builds the CLI parser for this script. Kept as its own function so
    it can also be imported and inspected (e.g. for --help text) without
    triggering __main__ execution."""
    import argparse

    parser = argparse.ArgumentParser(
        description=(
            "SoftMoE-SHAP training pipeline.\n\n"
            "Default (no arguments): runs ALL THREE R1#1 generalization-"
            "robustness protocols (learner-held-out / course-held-out / "
            "temporal-holdout) directly — equivalent to --holdout all. "
            "The main 25-run study is SKIPPED by default so this returns "
            "quickly (roughly 1/5 the main study's time in total) instead "
            "of waiting through the full ~18-22h run.\n\n"
            "Pass --holdout none --also-run-main (or just run main() "
            "directly) to run the main 25-run study instead."
        ),
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument(
        "--holdout", choices=list(HOLDOUT_CLI_ALIASES.keys()), default="all",
        help="'learner': GroupKFold-held-out by learner id (username). "
             "'course': GroupKFold-held-out by course id (course_id_encoded). "
             "'temporal': single chronological train/test split. "
             "'both': learner + course (legacy alias). "
             "'all': learner + course + temporal (DEFAULT). "
             "'none': skip all R1#1 protocols (use with --also-run-main, "
             "or call main() directly, to run only the main study).")
    parser.add_argument(
        "--also-run-main", action="store_true",
        help="Also run the main 25-run study (main()) in addition to any "
             "requested --holdout protocol(s). Default: when --holdout is "
             "not 'none', ONLY the holdout protocol(s) run.")
    parser.add_argument(
        "--k-folds", type=int, default=HOLDOUT_K_FOLDS_DEFAULT,
        help="Number of GroupKFold splits for the learner/course protocols.")
    parser.add_argument(
        "--train-frac", type=float, default=HOLDOUT_TRAIN_FRAC_DEFAULT,
        help="Fraction of chronologically-earliest enrollments used for "
             "training in the temporal-holdout protocol.")
    parser.add_argument(
        "--time-col", type=str, default=None,
        help="Explicit column name for the temporal-holdout protocol. If "
             "omitted, auto-detected from a list of common timestamp "
             "column names (see TIME_COLUMN_CANDIDATES near the top of "
             "this file).")
    parser.add_argument(
        "--data-path", type=str, default=DATA_PATH,
        help="Override the processed feature CSV path (main() itself "
             "always uses the DATA_PATH constant; this only affects the "
             "--holdout protocols).")
    parser.add_argument(
        "--cluster-path", type=str, default=CLUSTER_ASSIGN_PATH,
        help="Override the SHAP cluster-assignment CSV path (--holdout "
             "protocols only; see --data-path).")
    return parser
...
if __name__ == "__main__":
    ...
    # any such unrecognized arguments while still parsing our flags
    # correctly in both plain-Python and notebook contexts.
    _parser = _build_arg_parser()
    args, _unknown_args = _parser.parse_known_args()

    _requested_protocols = HOLDOUT_CLI_ALIASES[args.holdout]

    # Run the main 25-run study when no holdout was requested (original
    # default behavior), OR when the user explicitly opted in via
    # --also-run-main even though a holdout protocol was also requested.
    _run_main_study = (args.holdout == "none") or args.also_run_main

    print(f"\n{'='*70}")
    print(f"[STARTUP] --holdout={args.holdout!r}  --also-run-main={args.also_run_main}")
    if _requested_protocols:
        print(f"[STARTUP] Will run R1#1 protocol(s): {sorted(_requested_protocols)}")
    if _run_main_study:
        print("[STARTUP] Will also run the main 25-run study (~18-22h).")
    elif not _requested_protocols:
        print("[STARTUP] Nothing to do: --holdout none without --also-run-main. "
              "Pass --holdout all (or leave the default) to run the R1#1 suite, "
              "or add --also-run-main to run the main study.")
    print(f"{'='*70}\n")
