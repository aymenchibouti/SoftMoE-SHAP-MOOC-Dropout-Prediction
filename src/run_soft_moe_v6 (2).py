...
def run_group_holdout_protocol(group_col, protocol_name, k_folds=5):
    ...
    group_col : str
        ...
        (Confirm the exact column name in your processed CSV before
        running — see the assertion message below if it does not match.)
    protocol_name : str
...
def run_all_holdout_protocols():
    """Convenience wrapper: run both learner- and course-held-out protocols
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="SoftMoE-SHAP training pipeline. Default (no arguments) "
                     "runs the main 25-run enrollment-level study exactly as "
                     "before (v5 behavior, unchanged). Pass --holdout to "
                     "additionally/instead run the R1#1 learner-/course-"
                     "held-out generalization protocol.")
    parser.add_argument(
        "--holdout", choices=["learner", "course", "both", "none"], default="none",
        help="Run the group-held-out generalization protocol (R1#1) instead "
             "of/in addition to the main study. 'learner' groups by the "
             "'username' column; 'course' groups by 'course_id_encoded'; "
             "'both' runs both sequentially. Default 'none' preserves "
             "original v5 behavior.")
    parser.add_argument(
        "--holdout-only", action="store_true",
        help="If set together with --holdout, skip the main 25-run study "
             "entirely and only run the requested holdout protocol(s). "
             "By default both the main study and the holdout protocol run.")
    ...
    # any such unrecognized arguments while still parsing --holdout /
    # --holdout-only correctly, in both plain-Python and notebook contexts.
    args, _unknown_args = parser.parse_known_args()

    if args.holdout != "none" and args.holdout_only:
        if args.holdout in ("learner", "both"):
            run_group_holdout_protocol("username", "learner_held_out", k_folds=5)
        if args.holdout in ("course", "both"):
            run_group_holdout_protocol("course_id_encoded", "course_held_out", k_folds=5)
    else:
        main()
        if args.holdout in ("learner", "both"):
            run_group_holdout_protocol("username", "learner_held_out", k_folds=5)
